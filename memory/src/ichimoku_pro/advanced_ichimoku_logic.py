...
def calculate_advanced_ichimoku(df):
    ...
    # 2. 볼린저 밴드 (Bollinger Bands) 추가
    df['ma20_bb'] = df['Close'].rolling(window=20).mean()
    df['std20_bb'] = df['Close'].rolling(window=20).std()
    df['BB_upper'] = df['ma20_bb'] + (df['std20_bb'] * 2)
    df['BB_lower'] = df['ma20_bb'] - (df['std20_bb'] * 2)
